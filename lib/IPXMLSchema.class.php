...
class IPXMLSchema {
	...
	public function addComplexType( $type, $name = null, $parent = null ) {
		...
		if ( strtolower( substr( $type, 0, 6 ) ) == 'array(' || substr( $type, - 2 ) == '[]' ) {
			...
			if ( strtolower( substr( $type, 0, 6 ) ) == 'object' ) {//stdClass
				...
				if ( ! class_exists( $name ) ) {
					throw new WSDLException( "Error creating WSDL: no class found with the name '$name' / $type : $parent, so how should we know the structure for this datatype?" );
				}
	...
	public function addArray( $type, $parent ) {
		...
		$type = ( substr( $type, - 2 ) == '[]' ) ? substr( $type, 0, ( strlen( $type ) - 2 ) ) : substr( $type, 6,
			( strlen( $type ) - 7 ) );
	 ...
	 * @return \DomNode
	 */
	private function addElement( $name, $parent = null, $ns = null ) {
		if ( $ns ) {
