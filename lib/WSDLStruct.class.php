...
class WSDLStruct {
		...
		foreach ( (array) $this->services as $serviceName => $service ) {
			...
			foreach ( (array) $service->methods as $operation ) {
				...
				if ( strtolower( trim( $operation->return ) ) != 'void' ) {
					...
					$output->setAttribute( "name", $messageName );
					$this->addOutput( $this->bindingOperationTags[ $serviceName ][ $operationName ], false,
						$messageName );
	...
	private function addMessage( $name, $parts ) {
		...
		foreach ( (array) $parts as $partName => $partType ) {
			...
			if ( $t = IPXMLSchema::checkSchemaType( strtolower( $partType ) ) ) {
				...
				$partName = ( substr( $partType, - 2 ) == "[]" ) ? substr( $partType, 0,
						strpos( $partType, "[" ) ) . "Array" : $partType;
	 ...
	 */
	private function addInput( $parent, $input = true, $tag_name = null ) {
	 ...
	 */
	private function addOutput( $parent, $input = false, $tag_name ) {
		return $this->addInput( $parent, $input, $tag_name );
	}
