	...
	function parseTagLine( $tagLine ) {
		...
		switch ( strtolower( $tag ) ) {
			...
			case 'param':
				...
				break;
			case 'return':
				// added as per comment in component comments (http://bakery.cakephp.org/articles/view/a-component-to-help-creating-soap-services)
				if ( ! isset( $tagArr[1] ) ) {
					$tagArr[1] = 'void';
				}
			...
			case 'see':
				break;
			case 'since':
				$this->obj->since = trim( $tagArr[1] );
				break;
			case 'static':
				$this->obj->static = true;
				break;
			case 'throws':
				unset( $tagArr[0] );
